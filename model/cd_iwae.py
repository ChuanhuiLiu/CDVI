...
class CDIWAE(CDCVAE):
    """
    CDVI+IS: CD-CVAE with importance sampling for a tighter ELBO bound
    It requires a difference loss function, see metric.py
    """
    ...
    def compute_marginal_log_likelihood(self, x,y,delta, k=None, primative = "normal",beta=1):
        ...
        avg_log_marginal_likelihood = (torch.logsumexp(log_w, 1) -  np.log(k)).mean()

        return log_w,  avg_log_marginal_likelihood
    ...
    def fit(self,train_data, batch_size, num_epochs, learning_rate, format="pre",temperature=1,criterion="ctdpycox",early_stopper=True, patience=30):
        ...
        for epoch in range(num_epochs):
            ...
            if epoch % 2 == 0:
                ...
                if early_stopper == True and len(metric_list)> 9:
